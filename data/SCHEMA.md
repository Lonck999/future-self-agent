...
- `ongoing_notes_ref`：使用者持續記錄進度的筆記位置。每日規劃（證據導向換軌判斷）與每月確認 routine 都會讀取這裡的內容判斷進度（決策 4-4、5-1），初始訪談時需向使用者確認此欄位。物件欄位：
  - `type`：來源類型，例如 `git_repo`（目前唯一已實作的類型）
  - `url`：可存取的位置（例：git 倉庫 URL）
  - `scope`：要讀取的範圍說明。目前慣例是**依技能分資料夾**（例如 `Vue/` 放 Vue 相關技能進度、`Rust/` 放 Rust 相關技能進度）：每日規劃只需找技能名稱關鍵字對應的資料夾來掃描，不用整個 vault，效率較高、範圍也更精準；每月確認 routine 仍掃描整個 vault 做全面進度檢視。找不到明確對應資料夾時才退回掃描整個 vault。
